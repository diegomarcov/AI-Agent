...
%     debug_term(info, 'Known Map: ', Mapa),
...
remember_agent(Name, [previous_turn_action, attack(_)]):- 
	...
	member(agente(Name, Attack, Picking), A), % Si ya vimos al agente
	debug_term(info, 'Current attacking agent in sight: ', agente(Name, Attack, Picking)),
	subtract(A, [agente(Name, Attack, Picking)], NewA), % Lo sacamos temporalmente de la lista
	NewAttack is Attack + 1,
	replace(agentes(_), agentes(NewA)), % Guardamos la lista sin este agente
	insert_agent(Name, NewAttack, Picking), % Guardamos al agente con los nuevos datos
...
remember_agent(Name, [Attr, Val]):- 
	debug(warning, 'remember_agent: Case F: What the hell is this?'),
	concat(Attr, ' = ', Str),
	concat(Str, Val, Str2),
	debug(warning, Str2).
