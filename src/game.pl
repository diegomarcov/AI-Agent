% Things to do:
/*
Reflejar que los agentes pueden decidir no hacer nada en un dado turno
(tal vez basta con definir do(nothing) en las primitivas del agente,
que simplemente no haga nada. VER)
...
type_property(Type, Property):- type_property_direct(Type, Property).
...
ag_attr(AgName, unconscious, false):- ag_attr(AgName, unconscious_for, -1).
%ag_attr(_AgName, unconscious, true).
ag_attr(AgName, unconscious, true):- ag_attr(AgName, unconscious_for, UF),UF >= 0.

% Attribute climbing (derived attribute)
ag_attr(AgName, climbing, false):- ag_attr(AgName, climbing_for, -1).
ag_attr(AgName, climbing, true):- ag_attr(AgName, climbing_for, CF), CF >= 0.
