...
Mientras el agente esté aplicando esta estrategia, siempre que vea oro, intentará ir a buscarlo inmediatamente. Si existe un camino para llegar al oro por las porciones de mapa conocido, se apila la secuencia de acciones a realizar para llegar a dicho tesoro en la pila de acciones, se realizan inmediatamente para intentar recogerlo, y se continúa explorando. Si el tesoro es, por el momento, inalcanzable, se prosigue explorando como se planeó originalmente y el tesoro queda igualmente recordado en la "`memoria"' del agente.
...
Por último, en caso de que el mapa ya esté completamente explorado y Bugor no recuerde la existencia de tesoros pendientes, simplemente se desplazará al azar por el mapa.
